...
// Create an object called 'me' that has these keys: firstname, superHeroName, homeTown, superPowers, superPowerXP, profileImage. 
 let me = {
   firstName: "Philip",
   superHeroName: "Bigfoot Philip",
   homeTown: "Jacksonville",
   superPowers: ["skateboarding", "strength", "gas"],
...
function setColor(arr){
  ...
  splicedArr = arr.splice(0,3);
  

  for(let i = 0; i < splicedArr.length; i++){
    if(splicedArr[i] === "blue"){
      splicedArr[i] = '#4D4DFF'
    }
  }
  background(splicedArr[0], splicedArr[1], splicedArr[2]);
}
